...
def find_contour(img):
    ...
    if len(all_contours) >= 3:
        all_sorted_cnt = sorted(all_contours, key=cv2.contourArea)
        if (w*h)*0.0125 > cv2.contourArea(all_sorted_cnt[0]) or cv2.contourArea(all_sorted_cnt[len(all_contours)-1] >(w*h)*0.05):
            return None
        else:
            for contour in all_sorted_cnt:
                if (w*h)*0.0125 < cv2.contourArea(contour) < (w*h)*0.05:
                    selected_cnt.append(contour)
                    if len(selected_cnt) >= 3:
                        break
                else:
                    continue
        #selected_cnt = all_sorted_cnt[:4]
    elif 0 < len(all_contours) < 3:
        all_sorted_cnt = sorted(all_contours, key=cv2.contourArea)
        if (w*h)*0.0125 > cv2.contourArea(all_sorted_cnt[0]) or cv2.contourArea(all_sorted_cnt[len(all_contours)-1] >(w*h)*0.05):
            return None
        else:
            for contour in all_sorted_cnt:
                if (w*h)*0.0125 < cv2.contourArea(contour) < (w*h)*0.05:
                    selected_cnt.append(contour)
                    if len(selected_cnt) >= len(all_contours):
                        break
                    else:
                        continue
        #selected_cnt = all_sorted_cnt[:len(all_contours)]
    else:
        selected_cnt = None
        all_sorted_cnt = None
        print("no contours")
    return selected_cnt, all_sorted_cnt


def draw_contour_2(selected_cnt, canvas, contour_thickness):
    # for i, selected_cnt in enumerate(all_sorted_cnt):
    #     cv2.drawContours(canvas, selected_cnt, -1, [0, 255, 0], contour_thickness)
    for contours in selected_cnt:
        cv2.drawContours(canvas, contours, -1, [0, 255, 0], contour_thickness)


def fix_convex_defect(cnt_before_convex,img):
    ...
    if defects_before is not None:
        ...
        _, all_contours, _ = cv2.findContours(img, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        all_sorted_cnt = sorted(all_contours, key=cv2.contourArea)
        convex_fixed_cnt = all_sorted_cnt[0]
        ...
        convex_fixed_cnt = cnt_before_convex
        convex_fixed_img = img
    return convex_fixed_cnt, convex_fixed_img
...
def draw_box(selected_cnt):
    pos = []
    for cnt in selected_cnt:
        x, y, w, h = cv2.boundingRect(cnt)
        pos.append([x, y, w, h])
    return pos # Output pos = []

# Draw many on img_draw_box.
def draw_box_text(selected_cnt, img_draw_box, contour_thickness):
    pos = []
    for cnt in selected_cnt:
        colour = (255, 0, 0)
        x, y, w, h = cv2.boundingRect(cnt)
        ...
        pos.append([x, y, w, h])
    return pos, img_draw_box # Output pos=[]
...
def crop_possible_signs(selected_cnt, rgb_img):
    target_region_pos = draw_box(selected_cnt)
    print("target_region_pos",target_region_pos)
    border = 8
    target_region_img = crop_and_hist(rgb_img, target_region_pos, border)
    return target_region_img,target_region_pos


# Draw many cnt on one img_draw_box:
def text_box_on_img(selected_cnt,img_draw_box,text):
    target_region_pos = []
    target_region_pos, img_draw_box = draw_box_text(selected_cnt, img_draw_box, contour_thickness=2)
    for target_pos in target_region_pos:
        li = target_pos[0]
        x = li[0]
        y = li[1]
        color = (0,255,0)
...
video_path = 'sample_images/video_test_2.mov'
...
# Get the loose_white_mask
    ...
    output_red_list = [] #Cropped images
    cnt_red_list = []
    mask_red_list = [] #
    i = 0
    for red_mask in red_masks:
        selected_cnt_red, all_sorted_cnt_red = find_contour(red_mask)
        if all_sorted_cnt_red is not None:
            #draw_contour_2(red_mask, blank_canvas_red[i], 2)
            # Compare templet similarity:
            for red_contour in selected_cnt_red:
                cnt_pos = draw_box(red_contour)
                cnt_crop_img = crop_and_hist(red_mask, cnt_pos, border=4)

                similarity_level_ori_red = shape_compare(red_contour, octagon_template_cnt)
                # print("original similarity:", similarity_level_ori_red)
                if similarity_level_ori_red < 0.2:
                    target_region_img_red, _ = crop_possible_signs(red_contour, rgb_img)
                    output_red_list.append(target_region_img_red)
                    mask_red_list.append(red_mask)
                    cnt_red_list.append(red_contour)

                elif 0.2 < similarity_level_ori_red < 0.8:
                    approx_red_1 = cv2.approxPolyDP(red_contour, 0.04 * cv2.arcLength(red_contour, True), True)
                    print("length of approx_red_1:",len(approx_red_1))
                    if len(approx_red_1) == 3:
                        print("triangle")
                        output_red, _ = crop_possible_signs(red_contour, rgb_img)
                        ...
                        mask_red_list.append(red_mask)
                        cnt_red_list.append(red_contour)
                    else: # Fix convex defect and re-draw contour
                        convex_fixed_red_cnt = fix_convex_defect(red_contour, cnt_crop_img)
                        similarity_level_fix_red = shape_compare(convex_fixed_red_cnt, octagon_template_cnt)
                        # print("Fixed similarity:", similarity_level_fix_red)
                        if similarity_level_ori_red < 0.2:
                            output_red, _ = crop_possible_signs(convex_fixed_red_cnt, rgb_img)
                            output_red_list.append(output_red)
                            mask_red_list.append(convex_fixed_red_cnt)
                            cnt_red_list.append(red_contour)
                            ...
                            if len(approx_red_2) == 3:
                                print("triangle")
                                if cv2.contourArea(largest_fix_red_cnt) < (w * h) / 18:
            ...
            #         approx_red_2 = cv2.approxPolyDP(largest_fix_red_cnt, 0.04 * cv2.arcLength(largest_fix_red_cnt, True),
            #                                         True)
            #         print("length of approx_red_2:",len(approx_red_2))
            #         if len(approx_red_2) == 3:
            #             print("triangle")
            #             if (w * h)/100 < cv2.contourArea(largest_fix_red_cnt) < (w * h)/20:
            #                 output_red, _ = crop_possible_signs(convex_fixed_red_img, rgb_img)
    ...
    # Yellow:
        # Draw contours on red_canvas before fixing convex defects.
    blank_canvas_yellow_orange = [np.zeros(sample_img.shape, dtype=np.uint8),np.zeros(sample_img.shape, dtype=np.uint8)]
    blank_canvas_yellow_orange_2 = [np.zeros(sample_img.shape, dtype=np.uint8),np.zeros(sample_img.shape, dtype=np.uint8)]
    ...
    for yellow_orange_mask in yellow_orange_masks:
        ...
        if all_sorted_cnt_red is not None:
            ...
            if similarity_level_ori_yellow_orange < 0.2: # Directly crop out red signs from ori_img
                ...
                approx_yellow_orange_1 = cv2.approxPolyDP(largest_ori_yellow_orange_cnt,0.04*cv2.arcLength(largest_ori_yellow_orange_cnt,True),True)
                print(len(approx_yellow_orange_1))
                if len(approx_yellow_orange_1)==4:
                    print("dimond shape")
                    if cv2.contourArea(largest_ori_yellow_orange_cnt) < (w*h)/18:
                        output_yellow_orange,_ = crop_possible_signs(yellow_orange_mask, rgb_img)
                    ...
                    if similarity_level_fix_yellow_orange < 0.2:
                        ...
                        approx_yellow_orange_2 = cv2.approxPolyDP(largest_fix_yellow_orange_cnt,0.04*cv2.arcLength(largest_fix_yellow_orange_cnt,True),True)
    ...
    for white_mask in white_masks:
        ...
        if all_sorted_cnt_white is not None:
            ...
            if similarity_level_ori_white < 0.2:
                if cv2.contourArea(largest_ori_white_cnt) < (w * h) / 18:
                ...
                approx_white_1 = cv2.approxPolyDP(largest_ori_white_cnt, 0.04 * cv2.arcLength(largest_ori_white_cnt, True),
                                                  True)
                ...
                if len(approx_white_1) == 4:
                    ...
                    if similarity_level_fix_white < 0.2:
                        if cv2.contourArea(largest_fix_white_cnt) < (w * h) / 18:
                ...
                # Fix convex defect and re-draw contour:
                convex_fixed_white_img = fix_convex_defect(white_mask)
                #(convex_fixed_white_img, blank_canvas_white_2[q], 2)
                # plt.imshow(blank_canvas_white_2[q])
                # plt.show()
                largest_fix_white_cnt, _ = find_contour(convex_fixed_white_img)
                similarity_level_fix_white = shape_compare(largest_fix_white_cnt, rectangle_template_cnt)
                # print("Fixed similarity:", similarity_level_fix_white)
                if similarity_level_fix_white < 0.2:
                    if cv2.contourArea(largest_fix_white_cnt) < (w * h) / 9:
    ...
    for output_labeled_img in output_labeled_img_list:
        Image.fromarray(output_labeled_img).save('sample_images/labeled_video_3/output_labeled_frames_%d.jpg'%b)
        b += 1

    # Store all labeled frames:
    output_labeled_img_list.append(rgb_img_final)
    for output_labeled_img in output_labeled_img_list:
        plt.imshow(output_labeled_img)
        plt.show()
    # Within each frame, store all cropped images:
    p = 0
    for output_crop_red in output_red_list:
        Image.fromarray(output_crop_red).save('sample_images/cropped_video_2/output_crop_red_%d_%d.jpg'%(frame_i,p))
        p += 1
    o = 0
    for output_crop_yellow_orange in output_yellow_orange_list:
        Image.fromarray(output_crop_yellow_orange).save('sample_images/cropped_video_2/output_crop_yellow_orange_%d_%d.jpg'%(frame_i,o))
        o += 1
    u = 0
    for output_crop_white in output_white_list:
        Image.fromarray(output_crop_white).save('sample_images/cropped_video_2/output_crop_white_%d_%d.jpg'%(frame_i,u))
        u += 1
    frame_i += 1
