...
#     if cv2.contourArea(all_sorted_cnt[0]) < w*h*0.0125 or cv2.contourArea(all_sorted_cnt[0]) > w*h*0.05:
#         return None, None
...
#     [x,y,w,h] = region
#     # Expending a square border around the target region:
...
#     if crop_x[0]<0:
#         crop_x = [0, x+w+border]
#     else: crop_x = [x-border, x+w+border]
#     if crop_x[1]>img_w:
#         crop_x = [x-border, img_w]
#     else: crop_x = [x-border, x+w+border]
#     if crop_y[0]<0:
#         crop_y = [0, y+h+border]
#     else: crop_y = [y-border, y+h+border]
#     if crop_y[1]>img_h:
#         crop_y = [y-border, img_h]
#     crop_w = crop_x[1]-crop_x[0]
#     crop_h = crop_y[1]-crop_y[0]
#     # Get a square base on the larger border
#     if crop_w>crop_h:
#         diff = crop_w - crop_h
#         ori_crop_h = crop_h
#         crop_h = crop_w
#         crop_mask = np.zeros((crop_h,crop_w,3), dtype=np.uint8)
#         target_region_img = original_img[crop_y[0]-round(diff/2):crop_y[0]+ori_crop_h+round(diff/2), crop_x[0]:crop_x[0]+crop_w,:].copy()
#     else:
#         diff = crop_h - crop_w
#         ori_crop_w = crop_w
#         crop_w = crop_h
#         crop_mask = np.zeros((crop_h,crop_w,3), dtype=np.uint8)
#         target_region_img = original_img[crop_y[0]:crop_y[0]+crop_h, crop_x[0]-round(diff/2):crop_x[0]+ori_crop_w+round(diff/2),:].copy()
#    return target_region_img
